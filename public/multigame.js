// multigame.js - Handles the game logic and saves scores to the server.

// NOTE: This file assumes the necessary HTML structure (game-area, typed-word, 
// score displays, etc.) and the Socket.IO client library are loaded.
...
async function logMatchDetails(data, myMaxStreak) {
    ...
    try {
        const response = await fetch("http://localhost:3000/log_match", {
            method: "POST",
            headers: {
